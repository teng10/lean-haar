...
`O` and `H`, the gradient of the cost function has vanishing mean and variance
`2d ((Tr ρ² - d⁻¹)/(d² - 1)) (Tr O² / (d² - 1)) Tr H²`. -/
theorem observation57 (d : ℕ) (hd : 2 ≤ d) (ρ O H : Matrix (Fin d) (Fin d) ℂ)
    (hρ : ρ.trace = 1) (hO : O.trace = 0) (hH : H.trace = 0) :
    haarExp₂ d (gradient ρ O H) = 0 ∧
      haarVar₂ d (gradient ρ O H)
        = 2 * d * (((ρ * ρ).trace - (d : ℂ)⁻¹) / ((d : ℂ) ^ 2 - 1))
            * ((O * O).trace / ((d : ℂ) ^ 2 - 1)) * (H * H).trace := by
  haveI : NeZero d := ⟨by omega⟩
  haveI : Fact (2 ≤ d) := ⟨hd⟩
  -- the inner average over `U_B` of the gradient vanishes, since `Tr[H, X] = 0`
  have hmean_inner : ∀ UA : Matrix.unitaryGroup (Fin d) ℂ,
    ...
    rw [integral_const_mul, haar_integral_trace_conj ρ _, trace_commutator, mul_zero, zero_div,
      mul_zero]
  have hmean : haarExp₂ d (gradient ρ O H) = 0 := by
