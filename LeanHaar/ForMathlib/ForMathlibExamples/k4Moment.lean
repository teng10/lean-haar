...
lemma card_rep_swap (d : ℕ) :
  ...
  have e : {J : Fin 4 → Fin d // J = J ∘ (Equiv.swap (0:Fin 4) 1).symm} ≃ (Fin d × Fin d × Fin d) := by
...
lemma card_rep_dt (d : ℕ) :
    Fintype.card {J : Fin 4 → Fin d // J = J ∘ (Equiv.swap (0:Fin 4) 1 * Equiv.swap 2 3).symm} = d ^ 2 := by
  have e : {J : Fin 4 → Fin d // J = J ∘ (Equiv.swap (0:Fin 4) 1 * Equiv.swap 2 3).symm} ≃ (Fin d × Fin d) := by
...
lemma card_rep_3cyc (d : ℕ) :
    Fintype.card {J : Fin 4 → Fin d // J = J ∘ (Equiv.swap (0:Fin 4) 1 * Equiv.swap 1 2).symm} = d ^ 2 := by
  have e : {J : Fin 4 → Fin d // J = J ∘ (Equiv.swap (0:Fin 4) 1 * Equiv.swap 1 2).symm} ≃ (Fin d × Fin d) := by
...
lemma card_rep_4cyc (d : ℕ) :
    Fintype.card {J : Fin 4 → Fin d // J = J ∘ (Equiv.swap (0:Fin 4) 1 * Equiv.swap 1 2 * Equiv.swap 2 3).symm} = d ^ 1 := by
  have e : {J : Fin 4 → Fin d // J = J ∘ (Equiv.swap (0:Fin 4) 1 * Equiv.swap 1 2 * Equiv.swap 2 3).symm} ≃ (Fin d) := by
...
lemma weingartenGramNat_one (d : ℕ) (τ : Equiv.Perm (Fin 4)) :
  ...
  · rw [card_rep_4cyc, show numCyc (Equiv.swap (0:Fin 4) 1 * Equiv.swap 1 2 * Equiv.swap 2 3) = 1 from by decide]
...
lemma wg_rep_reduce (d : ℕ) (ν0 : Equiv.Perm (Fin 4)) :
    (∑ μ : Equiv.Perm (Fin 4), wgVal (d : ℂ) (μ⁻¹ * ν0) * (d : ℂ) ^ (numCyc μ))
      = ∑ j ∈ Finset.range 25,
          (Finset.univ.filter (fun i : Equiv.Perm (Fin 4) => 5 * numCyc i + ctIdx (i⁻¹ * ν0) = j)).card
            • (wgNum (j % 5) (d : ℂ) / ((d : ℂ) ^ 2 * Pden (d : ℂ)) * (d : ℂ) ^ (j / 5)) := by
  have hkey : ∀ μ : Equiv.Perm (Fin 4), 5 * numCyc μ + ctIdx (μ⁻¹ * ν0) ∈ Finset.range 25 := by
    intro μ
...
lemma wg_rep_id (d : ℕ) (hd : 4 ≤ d) :
  ...
  have hc : ∀ j ∈ Finset.range 25,
      (Finset.univ.filter (fun i : Equiv.Perm (Fin 4) => 5 * numCyc i + ctIdx (i⁻¹ * 1) = j)).card
...
lemma wg_rep_swap (d : ℕ) (hd : 4 ≤ d) :
    ∑ μ : Equiv.Perm (Fin 4), wgVal (d : ℂ) (μ⁻¹ * (Equiv.swap 0 1)) * (d : ℂ) ^ (numCyc μ) = 0 := by
  obtain ⟨hd0, hP⟩ := denom_facts d hd
  rw [wg_rep_reduce d (Equiv.swap 0 1)]
  have hc : ∀ j ∈ Finset.range 25,
      (Finset.univ.filter (fun i : Equiv.Perm (Fin 4) => 5 * numCyc i + ctIdx (i⁻¹ * (Equiv.swap 0 1)) = j)).card
...
lemma wg_rep_dt (d : ℕ) (hd : 4 ≤ d) :
    ∑ μ : Equiv.Perm (Fin 4), wgVal (d : ℂ) (μ⁻¹ * (Equiv.swap 0 1 * Equiv.swap 2 3)) * (d : ℂ) ^ (numCyc μ) = 0 := by
  obtain ⟨hd0, hP⟩ := denom_facts d hd
  rw [wg_rep_reduce d (Equiv.swap 0 1 * Equiv.swap 2 3)]
  have hc : ∀ j ∈ Finset.range 25,
      (Finset.univ.filter (fun i : Equiv.Perm (Fin 4) => 5 * numCyc i + ctIdx (i⁻¹ * (Equiv.swap 0 1 * Equiv.swap 2 3)) = j)).card
...
lemma wg_rep_3cyc (d : ℕ) (hd : 4 ≤ d) :
    ∑ μ : Equiv.Perm (Fin 4), wgVal (d : ℂ) (μ⁻¹ * (c[0,1,2] : Equiv.Perm (Fin 4))) * (d : ℂ) ^ (numCyc μ) = 0 := by
  obtain ⟨hd0, hP⟩ := denom_facts d hd
  rw [wg_rep_reduce d (c[0,1,2] : Equiv.Perm (Fin 4))]
  have hc : ∀ j ∈ Finset.range 25,
      (Finset.univ.filter (fun i : Equiv.Perm (Fin 4) => 5 * numCyc i + ctIdx (i⁻¹ * (c[0,1,2] : Equiv.Perm (Fin 4))) = j)).card
...
lemma wg_rep_4cyc (d : ℕ) (hd : 4 ≤ d) :
    ∑ μ : Equiv.Perm (Fin 4), wgVal (d : ℂ) (μ⁻¹ * (c[0,1,2,3] : Equiv.Perm (Fin 4))) * (d : ℂ) ^ (numCyc μ) = 0 := by
  obtain ⟨hd0, hP⟩ := denom_facts d hd
  rw [wg_rep_reduce d (c[0,1,2,3] : Equiv.Perm (Fin 4))]
  have hc : ∀ j ∈ Finset.range 25,
      (Finset.univ.filter (fun i : Equiv.Perm (Fin 4) => 5 * numCyc i + ctIdx (i⁻¹ * (c[0,1,2,3] : Equiv.Perm (Fin 4))) = j)).card
...
lemma ctIdx_conj (g κ : Equiv.Perm (Fin 4)) : ctIdx (g * κ * g⁻¹) = ctIdx κ := by
  native_decide +revert
lemma wgVal_conj (d : ℂ) (g τ : Equiv.Perm (Fin 4)) : wgVal d (g * τ * g⁻¹) = wgVal d τ := by
  unfold wgVal
  rw [ctIdx_conj]
/-- **Conjugation invariance of the class sum.** -/
lemma wg_Hsum_conj (d : ℕ) (g κ : Equiv.Perm (Fin 4)) :
    (∑ μ : Equiv.Perm (Fin 4), wgVal (d : ℂ) (μ⁻¹ * (g * κ * g⁻¹)) * (d : ℂ) ^ (numCyc μ))
...
`∑_{π} Wg(π⁻¹ρ) · Tr(V_d^†(σ) V_d(π)) = [σ = ρ]`, i.e. the matrix `Wg(σ⁻¹π)` is the inverse of
the Gram matrix `Tr(V_d^†(σ) V_d(π))`. This is the defining property of the Weingarten
...
noncomputable def coeff (d : ℕ) (O : Module.End ℂ (TensV d 4)) (π : Equiv.Perm (Fin 4)) : ℂ :=
