\subsection{Explicit Calculation of single qubit first moment}
...
\begin{remark}[Explicit construction of $2 \times 2$ unitaries]\label{rm:2x2UnitaryExplicitConstruction}
...
\begin{definition}[Parameterized single-qubit Haar measure]\label{def:SingleQubitHaarMeasure}
    Let $\mu_{2}$ denote the Haar measure on a single qubit, represented with vectors in $\mathbb{C}^{2}$.
    Define the {single-qubit Haar measure} in terms of the parameters given in Remark~\ref{rm:2x2UnitaryExplicitConstruction}, where
    \begin{equation}
    ...
    \begin{equation}\label{eq:NormalizedHaarMeas}
...
Given Def.~\ref{def:SingleQubitHaarMeasure}, we seek to show that
\begin{equation}
